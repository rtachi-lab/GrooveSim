...
def analyze_audio_file(path: str) -> dict[str, float | str]:
    audio = extract_audio_features(path)
    feature_map = compute_audio_feature_set(audio)
    result = combine_features(feature_map, source="audio")
    return result.to_dict()


def analyze_audio_bytes(data: bytes) -> dict[str, float | str]:
...
def analyze_midi_file(path: str, sr: int = 22050) -> dict[str, float | str]:
...
def analyze_midi_bytes(data: bytes, sr: int = 22050) -> dict[str, float | str]:
